...
const custom_actions = ['convert', 'recons', 'photogrammetry', 'render', 'thumbnail'];
ProcessQueue.prototype.add = function(scanId, button) {
  var self = this;
  var actions = [];
  for (var action_name of custom_actions) {
    var checked = $(`input:checkbox[name=${action_name}][data-id=${scanId}]`).is(':checked');
    if (checked) {
      actions.push(action_name);
    }
  }
  $.ajax({ url: this.url + 'add', data: { scanId: scanId, "overwrite": 1, "actions": JSON.stringify(actions) } })
    .done(function(resp) {
