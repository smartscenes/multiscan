...
function createActionButtons(scan) {
  ...
  if (scan.queueState && scan.queueState.queued) {
    ...
    if (scan.group === 'staging' && scan.path === scan.id) {
      var custom = ['convert', 'recons', 'photogrammetry', 'render', 'thumbnail'];
      var btnGroup = $('<div></div>').attr('class', 'btn-group row').attr('data-id', scan.id);
      ...
      var menu = $('<div></div>').attr('class', 'dropdown-menu').attr('role', 'menu');
      for (var i = 0; i < custom.length; i++) {
        menu.append('<div class="btn-group-toggle" data-toggle="buttons">')
          .append($('<label></label>').text(custom[i]).attr('class', 'btn btn-secondary active').attr('tabindex', '-1'))
          .append($('<input type="checkbox" checked>').attr('name', custom[i]).attr('data-id', scan.id));
      }
      btnGroup.append(menu);
      div.append(btnGroup);
      $("input:checkbox[name='photogrammetry']").prop('checked', false);
      $("input:checkbox[name='convert']").prop('checked', false);
    }
  }
  ...
  // div.append(createCheckBox('Procedures', ['convert', 'recons', 'photogrammetry', 'render', 'thumbnail']));
