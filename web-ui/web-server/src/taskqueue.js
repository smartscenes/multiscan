const fs = require('fs');
const BasicQueue = require('better-queue');
...
SqlStore.prototype.getTaskStates = function(n, fields, cb) {
  const self = this;
  const first = true;
  let q = self.adapter.knex(self.tableName).orderBy('lock', 'desc').orderBy('priority', 'DESC').orderBy('added', first ? 'ASC' : 'DESC');
...
MemoryStore.prototype.getTaskStates = function(n, fields, cb) {
  const self = this;
  const running = _.flatMap( self._running, (tasks, lockId) => {
  ...
  const queued = this._queue.map( taskId => {
    return {
      id: taskId,
  ...
  let all = _.concat([], running, queued);
...
Queue.prototype.list = function() {
  ...
  if (store._taskStates) {
    ...
    const tasks = store.getTaskStates();
    return tasks;
  }
};
...
Queue.prototype.loadState = function() {
  if (this._stateFile && fs.existsSync(this._stateFile)) {
    let state = fs.readFileSync(this._stateFile, 'utf-8');
    if (state) {
